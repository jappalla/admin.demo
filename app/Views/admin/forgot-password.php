<main id="main-content" class="admin-shell mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <section class="panel admin-panel admin-auth relative overflow-hidden border border-white/10 bg-base-900/60 backdrop-blur-xl">
        ...
        <div class="relative">
            ...
            <h1 class="mt-3 text-3xl font-extrabold tracking-tight text-white">Password dimenticata</h1>
            <p class="intro mt-2 text-white/70">Inserisci la tua email per ricevere il link di reset della password.</p>
            ...
            <form class="form-grid mt-6 rounded-2xl border border-white/10 bg-base-950/60 p-5 shadow-lg shadow-black/25" method="post" action="<?php echo e(route_url('admin/forgot-password')); ?>">
                <?php echo csrf_field(); ?>
                <label>
                    <span>Email</span>
                    <input type="email" name="email" autocomplete="email" required placeholder="admin@esempio.com">
                </label>
                <button class="button primary w-full sm:w-auto" type="submit">Invia link di reset</button>
            </form>
